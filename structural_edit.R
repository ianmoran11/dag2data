...
x.t1     = b_x.t0_x.t1*x.t0              + b_i.t0_x.t1*i.t0                                                                             %>%  rnorm(n = 1.00, mean = ., sd = .10*x.t0)
i.t1     = b_x.t1_i.t1*x.t1              + b_omega.t0_i.t1*omega.t0                                                                     %>%  rnorm(n = 1.00, mean = ., sd = i.t0*.10)
l.t1     = b_xi.t1_l.t1*xi.t1            + b_x.t1_l.t1*x.t1                                                                             %>%  rnorm(n = 1.00, mean = ., sd = .10*(b_xi.t1_l.t1*xi.t1+ b_x.t1_l.t1*x.t1))
omega.t1 = b_omega.t0_omega.t1*omega.t0  + b_xi.t1_omega.t1*xi.t1                                                                       %>%  rnorm(n = 1.00, mean = ., sd = 100)
