...
b_omega.t0_omega.t1 <- 1.00 %>%  rnorm(n = 1.00, mean = . ,sd = 0.00) # productivity persistence
...
x.t0                <- log(100000*rgamma(n = 1.00,shape = 1.018, rate = 0.0051)) # Firm size (employment distribution)
i.t0                <- .07 %>% rnorm(n = 1.00, mean = . ,sd = 0.0025)
t                   <- 0

# Each period - Exogenous ------------------------------------------------------

epsilon.t1          <- 0.00 %>%  rnorm(n = 1.00, mean = . ,sd = 0.025)
xi.t1               <- 0.00 %>%  rnorm(n = 1.00, mean = . ,sd = 0.025)

# Each period - Endogenous -----------------------------------------------------
t        <- t + 1
x.t1     <- b_x.t0_x.t1*x.t0              + b_i.t0_x.t1*i.t0                                                                             %>%  rnorm(n = 1.00, mean = ., sd = .01)
i.t1     <- b_x.t1_i.t1*x.t1              + b_omega.t0_i.t1*omega.t0                                                                     %>%  rnorm(n = 1.00, mean = ., sd = .01)
l.t1     <- b_xi.t1_l.t1*xi.t1            + b_x.t1_l.t1*x.t1                                                                             %>%  rnorm(n = 1.00, mean = ., sd = .01)
omega.t1 <- b_omega.t0_omega.t1*omega.t0  + b_xi.t1_omega.t1*xi.t1                                                                       %>%  rnorm(n = 1.00, mean = ., sd = .01)
y.t1     <- b_l.t1_y.t1*l.t1              + b_x.t1_y.t1*x.t1              + b_omega.t1_y.t1*omega.t1      + b_epsilon.t1_y.t1*epsilon.t1 %>%  rnorm(n = 1.00, mean = ., sd = 0)
